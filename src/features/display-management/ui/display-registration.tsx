...
import { Monitor, Package, MapPin, Calendar, Plus, Edit, Trash2, Check, X } from 'lucide-react'
import { useToast } from '@/shared/hooks/use-toast'
import { getMockProducts } from '@/entities/product/api'
import { getMockLocations, getMockParentLocations, updateMockLocation } from '@/entities/slot/api'
import { Product } from '@/entities/product/model'
import { Slot, ParentLocation } from '@/entities/slot/model'
...
export function DisplayRegistration({ userRole, currentStore }: DisplayRegistrationProps) {
  ...
  const availableLocations = locations.filter(location => 
    ...
    (location.currentQuantity === 0 || location.currentQuantity < location.maxQuantity)
  )

  // 사용 가능한 상품들 (재고가 있는 상품)
  const availableProducts = products.filter(product => 
    product.quantity > 0 && product.status !== 'displayed'
  )
  ...
  const handleCreateAssignment = () => {
    if (newAssignment.productId && newAssignment.locationId && newAssignment.quantity) {
      ...
      if (product && location) {
        const assignment: DisplayAssignment = {
          ...
          parentLocationName: parentLocations.find(pl => pl.id === location.parentId)?.name || location.parentId,
        ...
        const updatedLocation: Slot = {
          ...location,
          currentQuantity: location.currentQuantity + parseInt(newAssignment.quantity),
          product: {
            id: product.id,
            name: product.name,
            image: product.image,
  ...
  const handleSaveAssignment = () => {
    if (editingAssignment) {
      setDisplayAssignments(prev => prev.map(a => 
  ...
  const handleDeleteAssignment = (assignmentId: string) => {
    ...
    if (assignment) {
      ...
      if (location) {
        const updatedLocation: Slot = {
          ...location,
          currentQuantity: Math.max(0, location.currentQuantity - assignment.quantity),
          product: location.currentQuantity - assignment.quantity <= 0 ? undefined : location.product
        }
        updateMockLocation(updatedLocation)
        setLocations(getMockLocations())
      }
      ...
      // 상품 상태 복원
      setProducts(prev => prev.map(p => 
        p.id === assignment.productId ? { ...p, status: 'in_stock' as const } : p
      ))

      // 할당 제거
      setDisplayAssignments(prev => prev.filter(a => a.id !== assignmentId))
  ...
  // 전시 할당 상태 변경
  const handleStatusChange = (assignmentId: string, newStatus: 'active' | 'scheduled' | 'ended') => {
    setDisplayAssignments(prev => prev.map(a => 
  ...
  // 통계 정보
  const activeDisplays = displayAssignments.filter(a => a.status === 'active').length
  const scheduledDisplays = displayAssignments.filter(a => a.status === 'scheduled').length
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-600">총 전시 할당</p>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-600">총 전시 가치</p>
                <p className="text-2xl font-bold">{totalDisplayValue.toLocaleString()}원</p>
              </div>
                ...
                <TableRow key={assignment.id}>
                  ...
                  <TableCell>
                    <div className="flex items-center space-x-2">
                      ...
                      <span>{assignment.parentLocationName} - {assignment.layer}</span>
                    </div>
                  </TableCell>
                  <TableCell>{assignment.quantity}</TableCell>
                  <TableCell>{assignment.startDate.toLocaleDateString()}</TableCell>
                  <TableCell>{assignment.endDate ? assignment.endDate.toLocaleDateString() : '-'}</TableCell>
                  <TableCell>
                    <Badge variant={
                      assignment.status === 'active' ? 'default' :
                      assignment.status === 'scheduled' ? 'secondary' : 'outline'
                    }>
                      {assignment.status === 'active' ? '활성' :
                       assignment.status === 'scheduled' ? '예약' : '종료'}
                    </Badge>
                  </TableCell>
                  <TableCell>
                    <Badge variant={
                      ...
                      {assignment.priority === 'high' ? '높음' :
                       assignment.priority === 'medium' ? '보통' : '낮음'}
                    </Badge>
                  </TableCell>
                  <TableCell>
      ...
      <Dialog open={isAssignmentModalOpen} onOpenChange={setIsAssignmentModalOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>상품을 특정 위치에 전시하도록 할당하세요</DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <Label>상품</Label>
                <Select value={newAssignment.productId} onValueChange={(value) => setNewAssignment(prev => ({ ...prev, productId: value }))}>
                  <SelectTrigger>
              ...
              <div>
                <Label>위치</Label>
                <Select value={newAssignment.locationId} onValueChange={(value) => setNewAssignment(prev => ({ ...prev, locationId: value }))}>
                  <SelectTrigger>
                    <SelectValue placeholder="위치 선택" />
                  </SelectTrigger>
                  <SelectContent>
                    {availableLocations.map((location) => {
                      const parentLocation = parentLocations.find(pl => pl.id === location.parentId)
                      return (
                        <SelectItem key={location.id} value={location.id}>
                          {parentLocation?.name || location.parentId} - {location.layer} 층
              ...
              <div>
                <Label>우선순위</Label>
                <Select value={newAssignment.priority} onValueChange={(value) => setNewAssignment(prev => ({ ...prev, priority: value as 'high' | 'medium' | 'low' }))}>
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setNewAssignment(prev => ({ ...prev, startDate: new Date(e.target.value) }))}
                />
              </div>
              ...
              <div>
                ...
                <Input
                  type="date"
                  value={newAssignment.endDate ? newAssignment.endDate.toISOString().split('T')[0] : ''}
            ...
            <div className="flex gap-2">
              <Button onClick={handleCreateAssignment} className="flex-1">할당 생성</Button>
              <Button variant="outline" onClick={() => setIsAssignmentModalOpen(false)}>취소</Button>
            </div>
          </div>
        </DialogContent>
      </Dialog>
          ...
          {editingAssignment && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <Label>위치</Label>
                  <Input value={`${editingAssignment.parentLocationName} - ${editingAssignment.layer}`} disabled />
                </div>
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setEditingAssignment(prev => prev ? { ...prev, quantity: parseInt(e.target.value) || 0 } : null)}
                  />
                </div>

                <div>
                  <Label>우선순위</Label>
                  <Select value={editingAssignment.priority} onValueChange={(value) => setEditingAssignment(prev => prev ? { ...prev, priority: value as 'high' | 'medium' | 'low' } : null)}>
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                ...
                <div>
                  <Label>상태</Label>
                  <Select value={editingAssignment.status} onValueChange={(value) => setEditingAssignment(prev => prev ? { ...prev, status: value as 'active' | 'scheduled' | 'ended' } : null)}>
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                ...
                <div>
                  ...
                  <Input
                    type="date"
                    value={editingAssignment.startDate.toISOString().split('T')[0]}
                    onChange={(e) => setEditingAssignment(prev => prev ? { ...prev, startDate: new Date(e.target.value) } : null)}
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <Input
                  type="date"
                  value={editingAssignment.endDate ? editingAssignment.endDate.toISOString().split('T')[0] : ''}
              ...
              <div className="flex gap-2">
                <Button onClick={handleSaveAssignment} className="flex-1">저장</Button>
