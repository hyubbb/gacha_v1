...
import { getMockLocations, getMockParentLocations, updateMockLocation } from './mock-data'

import { Product, SuggestedLocation } from '@/shared/lib'

const SEVEN_DAYS_IN_MS = 7 * 24 * 60 * 60 * 1000;

export const getAvailableLocationsForProduct = (productPrice: number): SuggestedLocation[] => {
  const allLocations = getMockLocations();
  const allParentLocations = getMockParentLocations();
  const sevenDaysAgo = new Date(Date.now() - SEVEN_DAYS_IN_MS);

  const suggested: SuggestedLocation[] = [];

  for (const location of allLocations) {
    const parentLocation = allParentLocations.find(ps => ps.id === location.parentId);
...
export const assignProductToMockLocation = (locationId: string, product: Product, quantity: number) => {
  const allLocations = getMockLocations();
  const locationToUpdate = allLocations.find(s => s.id === locationId);
...
export const removeProductFromMockLocation = (locationId: string) => {
  const allLocations = getMockLocations();
  const locationToUpdate = allLocations.find(s => s.id === locationId);
