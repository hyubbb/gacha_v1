...
const availablePermissions = [
  ...
  },
  {
    id: 'slot_manage',
    name: '슬롯 관리',
    description: '슬롯 위치 설정 및 관리'
  },
  {
    id: 'display_manage',
    name: '전시 관리',
    description: '상품 전시 할당 및 관리'
  },
  {
    id: 'system_config',
    name: '시스템 설정',
    description: '시스템 전반 설정 변경'
  },
  {
    id: 'product_view',
    name: '상품 조회',
    description: '상품 정보 조회만 가능'
  },
...
export function UserManagement({ userRole }: UserManagementProps) {
  ...
  const filteredUsers = users.filter((user) => {
    ...
    const matchesStore =
      filters.store === 'all' || user.storeId === filters.store;
    const matchesStatus =
      filters.status === 'all' ||
  ...
  const handleCreateUser = () => {
    if (newUser.name && newUser.email) {
      const user: User = {
        ...
        storeName:
          newUser.role === 'branch'
            ? storeOptions.find((s) => s.id === newUser.storeId)?.name
            : undefined,
  ...
  const getPermissionDescription = (permissionId: string) => {
    return (
      availablePermissions.find((p) => p.id === permissionId)?.description || ''
    );
  };
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <Button onClick={() => setIsAddUserModalOpen(true)}>
              <Plus className="mr-1 h-4 w-4" />새 사용자
            </Button>
          </div>
        </CardHeader>
        <CardContent>
              ...
              {newUser.role === 'branch' && (
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(value) =>
                      setNewUser((prev) => ({ ...prev, storeId: value }))
                    }
                  >
                    <SelectTrigger>
                ...
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(value) =>
                      setEditingUser((prev) =>
                        prev
                          ? { ...prev, role: value as 'admin' | 'branch' }
                          : null
                      )
                    }
                  >
                  ...
                  {availablePermissions.map((permission) => (
                    ...
                    >
                      <Switch
                        ...
                        onCheckedChange={(checked) =>
                          togglePermission(editingUser.id, permission.id)
                        }
                      />
                      <Label className="text-sm">{permission.name}</Label>
                    </div>
      ...
      <Dialog open={isDeleteConfirmOpen} onOpenChange={setIsDeleteConfirmOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              정말로 {userToDelete?.name} 사용자를 삭제하시겠습니까? 이 작업은
              되돌릴 수 없습니다.
            </DialogDescription>
          </DialogHeader>
          <div className="flex gap-2">
